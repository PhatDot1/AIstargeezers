...
# Define functions for interacting with Airtable
def get_airtable_records(api_key, base_id, table_name):
    url = f'https://api.airtable.com/v0/{base_id}/{table_name}?filterByFormula=AND(NOT({{GitHub}} = ""), {{Status}} = "Run E")'
    headers = {
        'Authorization': f'Bearer {api_key}',
    }
    response = requests.get(url, headers=headers)
    if response.status_code != 200:
        logger.info(f"Error fetching records: {response.status_code}")
        return []
...
def update_airtable_record(api_key, base_id, table_name, record_id, fields):
    ...
    if response.status_code != 200:
        logger.info(f"Error updating record {record_id}: {response.status_code}")
        return False
...
def create_airtable_records(api_key, base_id, table_name, records):
    ...
    if response.status_code != 200:
        logger.info(f"Error creating records: {response.status_code}")
        return False
    logger.info(f"Successfully created records in bulk.")
    return True
...
def main():
    try:
        ...
        base_id = 'appyPlL933t1f8UJo'
        source_table_name = 'tbl8oecvN8XgstMMz'
        target_table_name = 'tblLMUMBCFle6gLzj'
        ...
        for record in records:
            ...
            if github_url and status == 'Run E':
